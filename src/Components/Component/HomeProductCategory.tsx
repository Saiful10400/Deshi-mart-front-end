...
console.log(data?.data)
    return (
        <div>
            ...
            {
                data?.data?.map(item=><Link className="font-normal text-lg bg-gray-300 px-3 rounded-md" key={item.categoryId} to={`/all-product?category=${item.name}`}>{item.name}</Link>)
            }
        </ul>
        </div>
    );
