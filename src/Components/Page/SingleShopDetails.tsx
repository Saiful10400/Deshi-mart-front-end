...
const SingleShopDetails = () => {
  ...
  const { data: storeAllProducts } = useGetAStoreAllProductNotDashboardQuery({
    id,
    page: 1,
    limit: 20000,
  });
  ...
  const isfollowIng = store?.followersId.find(
      ...
      <div className="mt-16 flex items-start gap-7 mb-10">
        ...
        <div className="w-full">
          <div>
            ...
            {(loggedInUser?.userId || store?.shopId) && (
              <button
                onClick={handleFollow}
                className="btn btn-sm mt-3 text-white bg-[#05adf4]"
              >
          ...
          <h1 className="text-xl mt-3 ">
            {store?._count?.followersId} Follower
          </h1>
